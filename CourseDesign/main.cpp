...
restart:
	...
	while (1)
	{
		MineSweeper.graph.gameDraw(); //执行一次play后重复进入循环反复贴图
		MineSweeper.operation.print(); //打印出当前剩余雷的数量
		MineSweeper.operation.ShowRecording();
		if (MineSweeper.play() == -1) //play函数自身会一直运行接收点击格子的信息，点击到格子后才返回
		{
			MineSweeper.operation.boom();
			MineSweeper.graph.gameDraw();  //踩到雷后全解密重新贴图展示所有雷
			mciSendString("close BGM ", 0, 0, 0);
			mciSendString("open ./爆炸音效.wav alias BGM", 0, 0, 0);
			mciSendString("play BGM", 0, 0, 0);
			MineSweeper.operation.Recording(0); //通关失败并记录
		...
		else if (ROW * COL - mineNum == count)  //找到了所有雷或者标记了所有雷
		{
			...
			mciSendString("play BGM", 0, 0, 0);
			MineSweeper.operation.Recording(1); //通关成功并记录
...
void  Operation::Recording(int pass) //记录游玩时间，是否通关,游戏耗时，写入文件    //记住每次运行程序显示  可读入数组然后判断第一行不为0的地方倒序输出
{
	timingStart = 0;
	FILE* fp;
